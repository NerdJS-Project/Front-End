...
export default function InstructorCourseView({ navigation }) {
    ...
    const apiConnection = new APIConnection();
          useLayoutEffect(() => {
            if(isFocused) {
              apiConnection.getModulesAndLessonInstructorCourseViewScreen().then(json => {
                setData(json);})
            }

            setFinalData(processAPIData())
            console.log("FINAL DATA DEBUG: " + finalData)
...
//Morph json from API into an array that we can use
         function processAPIData()
         {
            ...
            for(let i = 0; i< data.length; i++)
            {
                ...
                newModule["Module_Title"] = data.module_name;
                let newLessonArray = [];
                for(let j = 0; j< data[i].lessons.length; j++)
                {
                    newLessonArray[j] = data[i].lessons[j]
                }
  ...
  const list = [
    {
      ...
      onPress: () => {
        {
          setListDataSource(jsonRootData[0].courseContent);
        }
        {
          setCourseTitle(jsonRootData[0].courseName);
        }
      },
    },
    {
      ...
      onPress: () => {
        {
          setListDataSource(jsonRootData[2].courseContent);
        }
        {
          setCourseTitle(jsonRootData[2].courseName);
        }
      },
    },
    {
      ...
      onPress: () => {
        {
          setListDataSource(jsonRootData[1].courseContent);
        }
        {
          setCourseTitle(jsonRootData[1].courseName);
        }
      },
    },
    ...
    >
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.header}>
          ...
          >
            <FontAwesome5 size={20} name={"bars"} color={"white"} />
          </TouchableOpacity>
        </View>
      ...
      <BottomSheet
        style={{ borderRadius: 100, marginTop: 90, width: 140 }}
        modalProps={{}}
        isVisible={isVisible}
      >
